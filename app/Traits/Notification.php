...
trait Notification
{
	...
	): void
	{
		dispatch(function () use ($receivers, $data, $userIds) {
			if (empty($receivers)) {
				\Log::channel('firebase')->error('No receivers provided for notification');
				return;
			}
			...
			if (!empty($userIds)) {
				try {
					...
					\Log::channel('firebase')->error('Failed to store notification: ' . $e->getMessage());
				}
			}
			...
			try {
				...
				if (empty($authToken)) {
					\Log::channel('firebase')->error('Failed to get Firebase auth token');
					return;
				}
			} catch (\Exception $e) {
				\Log::channel('firebase')->error('Error getting Firebase auth token: ' . $e->getMessage());
				return;
			}
			...
			if (empty($processedReceivers)) {
				\Log::channel('firebase')->error('No valid receivers after processing');
				return;
			}
			...
			foreach ($processedReceivers as $token) {
				try {
					...
					if (!$response->successful()) {
						\Log::channel('firebase')->error('Failed to send notification to token', [
						...
						if ($error && (
							...
							try {
								...
								if ($user) {
									...
									\Log::channel('firebase')->info("Removed invalid token from user #{$user->id}");
								}
							} catch (\Exception $e) {
								\Log::channel('firebase')->error('Failed to remove invalid token: ' . $e->getMessage());
							}
						}
					}
				} catch (\Exception $e) {
					\Log::channel('firebase')->error('Exception sending notification to token', [
						'token_prefix' => substr($token, 0, 15) . '...',
						'error' => $e->getMessage()
					]);
				}
		...
		try {
			...
			if (!empty($shopId)) {
				$sellersFirebaseTokens = User::with([])
					...
					->whereHas('shop', fn($q) => $q->where('id', $shopId))
					->where('active', 1)
					->whereNotNull('firebase_token')
					->pluck('firebase_token', 'id')
					->toArray();
			...
			// Send notification to admins and sellers (less verbose logging)
			if (!empty($allTokens)) {
				$notificationData = [
					'id' => $order->id,
					'status' => $order->status,
					...
					'body' => __('errors.' . ResponseError::NEW_ORDER, ['id' => $order->id], $this->language)
				];
			...
			if ($order->user && !empty($order->user->firebase_token)) {
				...
				if (!empty($userTokens)) {
					$notificationData = [
						...
						'body' => __('Your order #:id has been received!', ['id' => $order->id], $this->language)
					];
