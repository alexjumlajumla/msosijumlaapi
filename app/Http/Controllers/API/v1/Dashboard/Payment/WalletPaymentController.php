...
class WalletPaymentController extends PaymentBaseController
{
    ...
    public function paymentTopUp(PaymentTopUpRequest $request): JsonResponse
    {
        ...
        try {
            ...
            if ($payment->tag === Payment::TAG_STRIPE) {
                ...
                $response = $this->selcom($payload, $totalPrice);
    ...
    public function selcom(array $payload, int|float $totalPrice): array
    {
        ...
        /** @var User $user */
        $user     = auth('sanctum')->user();
        $redirectUrl = "$this->host/wallet-success";
        $cancelUrl = "$this->host/wallet-success";
        $webhookUrl = "$this->host/wallet-success";
        ...
        $response =  $api->cardCheckoutUrl([
            ...
            'state' =>  'Dar Es Salaam',
            'city' =>  'Dar Es Salaam',
            'billing_phone' => $this->formatPhone($user->phone ?? $user->phone),
            'currency' => $currency,
            'items' => 1,
        ]);
